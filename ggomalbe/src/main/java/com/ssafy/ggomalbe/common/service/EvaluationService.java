...
public class EvaluationService {
     ...
     * @return score: originText와 아이의 음성을 비교하여 도출한 점수
    ...
    //speechsuper 사용하여 평가진행
    public Mono<EvaluationDto> evaluationV2(FilePart filePart, String originText) {
        return toBuffer(filePart)
                .flatMap(buffer -> naverCloudClient.soundToText(buffer))
                .flatMap(sttText -> {
                    return speechSuperService.evaluation(filePart, originText)
                            .map(result->{
                                float score = result.getResult().getPronunciation();

                                return EvaluationDto.builder()
                                        .pronunciation(sttText)
                                        .score(score)
                                        .originText(originText)
                                        .build();
                            });
